...
try {
  // Total buku diterima
  $stmt = $pdo->query(
    "SELECT SUM(jumlah) FROM donasi WHERE status = 'diterima'",
  );
  $total_buku = $stmt->fetchColumn() ?: 0;

  // Total pendonasi unik yang donasinya sudah diterima
  $stmt = $pdo->query(
    "SELECT COUNT(DISTINCT user_id) FROM donasi WHERE status = 'diterima'",
  );
                ...
                <div class="row g-4">
                    <div class="col-sm-6">
                        <div class="stat-box-editorial text-center text-sm-start">
                            ...
                            <h2 class="display-5 fw-bold text-dark mb-0 mt-1"><?= number_format(
                              $total_buku,
                            ) ?>+</h2>
                        </div>
                    </div>
                            ...
                            <div>
                                <div style="height: 220px; overflow: hidden; background-color: #f3f4f6; display: flex; align-items: center; justify-content: center; position: relative;">
                                    <?php if (
                                      $buku["foto"] &&
                                      file_exists(
                                        "assets/uploads/" . $buku["foto"],
                                      )
                                    ): ?>
                                        <img src="assets/uploads/<?= htmlspecialchars(
                                          $buku["foto"],
                                        ) ?>" class="card-img-top w-100 h-100 object-fit-cover" alt="<?= htmlspecialchars(
  $buku["judul_buku"],
) ?>">
                                    <?php else: ?>
                                ...
                                <div class="card-body">
                                    <span class="small fw-bold text-uppercase mb-1 d-block" style="color: var(--color-primary);"><?= htmlspecialchars(
                                      $buku["nama_kategori"],
                                    ) ?></span>
                                    <h6 class="card-title fw-bold text-dark line-clamp-2 mb-2"><?= htmlspecialchars(
                                      $buku["judul_buku"],
                                    ) ?></h6>
                                </div>
                            </div>
                            <div class="px-3 pb-3">
                                <div class="pt-2 border-top d-flex justify-content-between align-items-center text-muted small">
                                    <span>Kondisi: <strong class="text-dark text-capitalize"><?= str_replace(
                                      "_",
                                      " ",
                                      htmlspecialchars($buku["kondisi"]),
                                    ) ?></strong></span>
                                </div>
                            </div>
                        </div>
